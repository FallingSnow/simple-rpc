...
export default class Server extends WebSocket.Server {
    ...
    async sendSocket({type = "c", socket, namespace, data, response = true}) {
        const id = socket.messageId();

        await socket.send(JSON.stringify({type, namespace, id, data}));
    ...
    async processMessage(socket, message) {
        ...
        // console.debug(`Got ${type} for ${namespace}`);
